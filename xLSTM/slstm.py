...
class sLSTMCell(nn.Module):
    ...
    def forward(self, input, hx):
        ...
        # === 【關鍵修改】混合閘控機制 (Hybrid Gating) ===
        
        # 1. Input Gate: 維持 xLSTM 的指數特性，但加上 Clamp 防止單次過大
        i = torch.exp(torch.clamp(i, max=3.0)) 
        
        # 2. Forget Gate: 【改用 Sigmoid】
        # 這是為了強制讓 f 在 0~1 之間，確保記憶體 'c' 不會發生指數爆炸
        f = torch.sigmoid(f) 
